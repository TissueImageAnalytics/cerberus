...
def remap_label(pred, by_size=False, ds_factor=None):
	...
	if by_size:
		pred_size = np.bincount(pred.ravel())[1:]
		pred_id = np.argsort(pred_size)[::-1] + 1
	else:
		pred_id = np.arange(1, len(pred_id) + 1)
...
def get_inst_centroid(inst_map):
	labels, _, stats, centroids = cv2.connectedComponentsWithStats(inst_map.astype(np.uint8))
	inst_centroid_list = centroids[1:].astype(np.int32)
...
def stain_entropy_otsu(img):
	...
	entropy = h_entropy + e_entropy - d_entropy
	# otsu threshold
