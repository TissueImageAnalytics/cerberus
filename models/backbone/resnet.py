...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        numerical_check = lambda x: torch.isnan(x) | torch.isinf(x)

        out = self.conv1(x)
        # assert numerical_check(out).any(axis=-1).sum() == 0
        # print(numerical_check(out).any(axis=-1).sum(), x.shape[0])
        out = self.bn1(out)
        out = self.relu(out)
...
def resnext101_32x4d(pretrained=False, progress=True, **kwargs):
    ...
    kwargs["width_per_group"] = 4
    return _resnet(
        "resnext101_32x4d", Bottleneck, [3, 4, 23, 3], False, progress, **kwargs
    )
